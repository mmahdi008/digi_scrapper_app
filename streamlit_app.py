...
def _get_json(url, use_scrapingbee=False, scrapingbee_api_key=None):
    ...
    if use_scrapingbee and scrapingbee_api_key:
        ...
        params = {
            ...
            'url': url,
            'render_js': 'false',  # No JavaScript rendering needed for API endpoints
            'premium_proxy': 'true',  # Use premium proxies
        }
        
        headers = {"Accept": "application/json"}
        
        print(f"Fetching via ScrapingBee: {url}")  # Debug log
        
        for attempt in range(RETRIES):
            try:
                print(f"Attempt {attempt + 1}/{RETRIES} (via ScrapingBee)")
                r = requests.get(scrapingbee_url, params=params, headers=headers, timeout=TIMEOUT)
                print(f"Response status: {r.status_code}")
                
                if r.status_code == 200:
                    # ScrapingBee returns the content from the target URL
                    try:
                        # Try to parse as JSON
                        json_data = r.json()
                        print(f"✅ Successfully fetched JSON via ScrapingBee")
                        return json_data
                    except ValueError:
                        # If not JSON, try parsing text
                        try:
                            import json
                            ...
                            if 'error' in r.text.lower()[:200] or 'scrapingbee' in r.text.lower():
                                error_msg = f"ScrapingBee returned error: {r.text[:200]}"
                                print(f"⚠️ {error_msg}")
                                raise RuntimeError(error_msg)
                            else:
                                raise RuntimeError(f"ScrapingBee returned non-JSON content: {r.text[:200]}")
                
                elif r.status_code == 403:
                    error_msg = f"HTTP 403 - ScrapingBee API key may be invalid or quota exceeded"
                    print(f"⚠️ {error_msg}")
                    raise RuntimeError(error_msg)
                else:
                    error_msg = f"HTTP {r.status_code} from ScrapingBee"
                    print(f"⚠️ {error_msg}")
                ...
                # Re-raise RuntimeErrors (like 403, invalid response)
                raise
        ...
        print(f"Fetching URL directly: {url}")  # Debug log
...
def scrape_from_plp(plp_url, target_count, progress_bar, status_text, use_scrapingbee=False, scrapingbee_api_key=None):
    ...
    if use_scrapingbee:
        print(f"Using ScrapingBee proxy with API key: {scrapingbee_api_key[:10]}...")  # Debug log
    ...
    if use_scrapingbee:
        ...
        if 'scrapingbee' in st.secrets and 'api_key' in st.secrets.scrapingbee:
            ...
            scrapingbee_api_key = st.text_input(
                "ScrapingBee API Key",
                type="password",
                help="Enter your ScrapingBee API key. Get one from https://www.scrapingbee.com/",
                placeholder="Enter your API key here"
            )
            if scrapingbee_api_key:
                scrapingbee_api_key = scrapingbee_api_key.strip()
                if scrapingbee_api_key:
...
if scrape_button and not st.session_state.scraping:
    if not plp_url:
        ...
        try:
            ...
            # Show proxy status
            if use_scrapingbee and scrapingbee_api_key:
                st.success(f"🔄 Using ScrapingBee Proxy")
            else:
